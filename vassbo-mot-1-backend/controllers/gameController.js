...
const joinGame = async (req, res) => {
  ...
  console.log(
    `Attempting to join game: ${gameCode} with playerName: ${playerName}`,
  );
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while adding the player." });
  }
};
...
const addQuestion = async (req, res) => {
  ...
  console.log(
    `Adding question to game: ${gameCode}. Question: ${question.text}`,
  );
...
const submitGuess = async (req, res) => {
  ...
  try {
    ...
    const allGuessesSubmitted = game.players.every(
      (player) => game.answers[player.name] !== undefined,
    );
    if (allGuessesSubmitted) {
      game.setPhase(3); // Move to the next phase
      await Game.saveGame(gameCode, game);
      req.app
        .get("io")
        .to(gameCode)
        .emit("updatePhase", { phase: 3, gameCode });
    }
...
const setCorrectAnswer = async (req, res) => {
  ...
  try {
    ...
    // Move to the leaderboard phase
    game.setPhase(4);
    await Game.saveGame(gameCode, game);
    io.to(gameCode).emit("updatePhase", { phase: 4, gameCode });
...
const setNextPhase = async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: "Error moving to next phase." });
  }
...
const nextQuestion = async (req, res) => {
  ...
  try {
    ...
    if (game.currentQuestionIndex + 1 < game.questions.length) {
      ...
      game.phase = 1; // e.g. Phase 1 => "Present Question"
    } else {
    ...
    res.json({ message: "Moved to next question.", game });
  } catch (error) {
...
const updateQuestion = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Question at index ${index} updated successfully for game: ${gameCode}`,
    );
